"""In-memory ChatKit thread store used for local development and testing."""
...
class ThreadStore(Store[dict[str, Any]]):
    """Lightweight `Store` implementation compatible with the ChatKit server.

    The implementation mirrors the approach used in the ChatKit demo backend:
    thread metadata is stored separately from thread items, and attachments are
    intentionally unsupported for security reasons. Data is ephemeral—restart
    the process to clear state.
    """
    ...
    def _coerce_thread_metadata(thread: ThreadMetadata | Thread) -> ThreadMetadata:
        """Return a metadata-only copy, stripping any eager-loaded items."""
    ...
    async def save_attachment(
        ...
        raise NotImplementedError(
            "ThreadStore does not persist attachments. Provide a Store implementation "
    ...
    async def load_attachment(
        ...
        raise NotImplementedError(
            "ThreadStore does not load attachments. Provide a Store implementation "
            "that enforces authentication and authorization before enabling uploads."
        )

    async def delete_attachment(self, attachment_id: str, context: dict[str, Any]) -> None:
        raise NotImplementedError(
            "ThreadStore does not delete attachments because they are never stored."
        )
